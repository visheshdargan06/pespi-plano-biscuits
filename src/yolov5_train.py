...
class Train_YOLOv5:
    def __init__(self, model_type):
        ...
        if self.model_type == 'rackrow':
            ...
            self.model_config = self.config["yolov5_train_packets"]
        self.weights = self.model_config['weights']  
        self.cfg = self.model_config['cfg']
        self.data = self.model_config['data']
        self.hyp = self.model_config['hyp']
        self.epochs = self.model_config['epochs']
        self.batch_size = self.model_config['batch_size']
        self.imgsz = self.model_config['imgsz']
        self.resume = self.model_config['resume']
        self.rect = self.model_config['rect']
        self.nosave = self.model_config['nosave']
        self.noval = self.model_config['noval']
        self.noautoanchor = self.model_config['noautoanchor'] 
        self.evolve = self.model_config['evolve']   
        self.bucket = self.model_config['bucket']  
        self.cache = self.model_config['cache']
        self.image_weights = self.model_config['image_weights']
        self.device = self.model_config['device']
        self.multi_scale = self.model_config['multi_scale']
        self.single_cls = self.model_config['single_cls']
        self.adam = self.model_config['adam']
        self.sync_bn = self.model_config['sync_bn']
        self.workers = self.model_config['workers']
        self.project = self.model_config['project']
        self.name = self.model_config['name']
        self.exist_ok = self.model_config['exist_ok']
        self.quad = self.model_config['quad']
        self.linear_lr = self.model_config['linear_lr']
        self.label_smoothing = self.model_config['label_smoothing']
        self.patience = self.model_config['patience']
        self.freeze = self.model_config['freeze']
        self.save_period = self.model_config['save_period']
        self.local_rank = self.model_config['local_rank']
        self.entity = self.model_config['entity']
        self.upload_dataset = self.model_config['upload_dataset']
        self.bbox_interval = self.model_config['bbox_interval']
        self.artifact_alias = self.model_config['artifact_alias']
