...
images_location = "/media/premium/common-biscuit/main/planogram_biscuit/data/raw/blob_copy/"
pathlib.Path(images_location).mkdir(parents=True, exist_ok=True)
p = subprocess.run(["/media/premium/common-biscuit/main/azcopy", 
                "copy", 
                "https://pepsiplanobiscuits.blob.core.windows.net/common/model_files_planogram/planogram/data/processed?sp=racwl&st=2021-12-02T10:03:30Z&se=2021-12-02T18:03:30Z&sv=2020-08-04&sr=c&sig=rIrzZiyKtsD%2FZxkzkWVJoIQS9fkd5N88AiQWWFRiGCA%3D", 
                images_location, 
                "--recursive"])
...
#     images_location = os.path.join(config.get('path').get('blob_base_dir'),config.get('data_path').get('images_dir'))
#     pathlib.Path(images_location).mkdir(parents=True, exist_ok=True)
#     p = subprocess.run([config.get('path').get('blob_base_dir')+"/./azcopy", 
#                     "copy", 
